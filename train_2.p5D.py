...
from Dataset.dataset_2p5D import DenoiseDataset, ValDataset
...
def train(train_noisy_files, train_clean_files, train_mask_files, num_epochs, save_model_path, noise_level,val_noisy_files, val_clean_files, val_mask_files, resume_checkpoint=None):
    ...
    if resume_checkpoint is not None and os.path.isfile(resume_checkpoint):
        ...
        # 兼容处理：如果你传入的是纯权重的.pth文件，它没有optimizer_state_dict，这里做个保护
        if 'optimizer_state_dict' in checkpoint:
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
            start_epoch = checkpoint['epoch'] + 1
            loss_log = checkpoint.get('loss_log', [])
            print(f"=> 成功加载完整检查点！将从 Epoch {start_epoch + 1} 继续训练。")
        else:
            print("=> 警告：该文件仅包含模型权重，未包含优化器状态。将从 Epoch 1 重新开始优化过程。")
    else:
        print("=> 未指定检查点或文件不存在，将从头开始训练。")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for noisy,clean,mask in loop:
            ...
            loop.set_postfix(
                ...
                loss=loss.item(),
                avg_loss=avg_loss,
                lr=optimizer.param_groups[0]['lr'] # 实时显示当前学习率
            )
        ...
        if (epoch+1) % 10 == 0:
            ...
            # 逢 10 额外保存一个带轮数的 checkpoint 以作历史备份
            ...
            with torch.no_grad():
                    for noisy, clean, mask in val_loader:
                        ...
                        noisy_center = noisy_np[1, :, :]
                        
                        denoised = denoised * mask + (-1) * (1 - mask)  # 背景区域设置为-1（黑色）
                        plot_val(clean, noisy_center, denoised,mask, noise_level,epoch+1, save_val_dir)
 
    # 训练全部完成后，保存纯净版权重
    torch.save(model.state_dict(), f'{save_model_path}/model_{noise_level}%noise_final.pth')
    print(f'Training completed. Final model saved at {save_model_path}/model_{noise_level}p_noise_final.pth')
