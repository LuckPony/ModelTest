...
class DenoiseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        max = stat_dict[file_id.item()]["max"]
        min = stat_dict[file_id.item()]["min"]
        #归一化到[-1,1]
...
class TestDataset(Dataset):
    def __init__(self, noisy_files, clean_files):
        self.noisy_img = []
        self.clean_img = []
        for n_file, c_file in zip(noisy_files, clean_files):
    ...
    def __getitem__(self, idx):
        ...
        with open("normalize/test_minmax.json","r") as f:
            stat_dict = json.load(f)
        max = stat_dict[str(f_id)]["noisy"]["max"]
        min = stat_dict[str(f_id)]["noisy"]["min"]
        #进行归一化
        noisy = (noisy - min) / (max - min)
        clean = (clean - min) / (max - min)
        noisy = 2*noisy - 1
